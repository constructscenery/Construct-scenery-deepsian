...
function PublicQr({ token, filename }: { token: string; filename: string }) {
  const [src, setSrc] = useState('');
  useEffect(() => {
    QRCode.toDataURL(safetyHealthApi.publicUrl(token), { width: 260, margin: 1 }).then(setSrc).catch(() => setSrc(''));
  }, [token]);
  return src ? <a href={src} download={filename} title="Download QR code"><img src={src} alt="Certificate QR code" className="w-64 h-64" /></a> : null;
}
...
export default function PublicSafetyHealthPage() {
  ...
  return <main className="min-h-screen bg-slate-50 p-4 md:p-8">
    <div className="max-w-6xl mx-auto space-y-5">
      <header className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">CS HQ · Safety & Health</p>
          </div>
        </div>
        <h1 className="text-2xl font-semibold text-slate-900 mt-5">Safety & Health Certificates</h1>
      ...
      {qrDocument && <div className="fixed inset-0 z-50 flex items-center justify-center p-4"><div className="absolute inset-0 bg-black/40" onClick={() => setQrDocument(null)} /><div className="relative bg-white rounded-xl shadow-xl p-6 text-center"><button onClick={() => setQrDocument(null)} className="absolute top-3 right-3 text-slate-400"><X size={18} /></button><h2 className="font-semibold text-slate-800 pr-5">Shareable QR Code</h2><p className="text-xs text-slate-500 mt-1 max-w-64 truncate">{qrDocument.file_name}</p><div className="mt-4"><PublicQr token={qrDocument.public_token!} filename={`${qrDocument.file_name}-qr.png`} /></div><a href={safetyHealthApi.publicUrl(qrDocument.public_token!)} target="_blank" rel="noreferrer" className="text-sm text-blue-600 hover:underline">Open public certificate</a></div></div>}
    </div>
  </main>;
}
